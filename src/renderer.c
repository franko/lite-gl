...
/* ------------------------------------------------------------------------
 * blit_glyph: single point where pixel-by-pixel blending of glyph bitmaps
 * onto the destination surface happens.  It is used by both the ASCII
 * “fast-path” and the HarfBuzz path, avoiding ~150 lines of duplicated
 * code.  Any future changes to blending need to be done only here.
 * --------------------------------------------------------------------- */
                       ...
                       bool is_subpixel)
{
  ...
  for (int row = 0; row < glyph_h; ++row) {
    ...
    for (int col = 0; col < glyph_w; ++col, ++x_col, ++dst) {
      ...
      // Check to skip the pixel if it is zero.
      uint8_t src_a = is_subpixel ? (src_r | src_g | src_b) : src_r;
      if (src_a == 0) continue;
...
/* ------------------------------------------------------------------------
 * Helper: draw underline and/or strikethrough decorations for a text run.
 * `start_px` and `end_px` are PEN positions already scaled (pixels).
 * `y_text` is the original, un‑scaled y argument passed to ren_draw_text*.
 * --------------------------------------------------------------------- */
static void draw_text_decoration(RenSurface *rs, RenFont *font,
                                 double start_px, double end_px,
                                 int y_text, RenColor color)
{
  if (!font || color.a == 0) return;

  const int surface_scale = rs->scale;
  double start_unscaled  = start_px / surface_scale;
  double width_unscaled  = (end_px  - start_px) / surface_scale;

  bool underline     = font->style & FONT_STYLE_UNDERLINE;
  bool strikethrough = font->style & FONT_STYLE_STRIKETHROUGH;
  if (!underline && !strikethrough) return;

  if (underline) {
    ren_draw_rect(rs,
      (RenRect){ start_unscaled,
                 y_text + font->height - 1,
                 width_unscaled,
                 font->underline_thickness * surface_scale },
      color);
  }
  if (strikethrough) {
    ren_draw_rect(rs,
      (RenRect){ start_unscaled,
                 y_text + font->height / 2,
                 width_unscaled,
                 font->underline_thickness * surface_scale },
      color);
  }
                            ...
                            const hb_glyph_position_t pos, RenColor color)
{
  ...
  for (int row = 0; row < slot->bitmap.rows; ++row) {
    ...
    for (int col = 0; col < (int)(slot->bitmap.width / byte_width); ++col) {
      ...
      // Check to skip the pixel if it is zero.
      uint8_t src_a = (font->antialiasing == FONT_ANTIALIASING_SUBPIXEL) ? (src_r | src_g | src_b) : src_r;
  ...
  while (text < end) {
    ...
    if(!last) last = font;
      ...
      if (underline || strikethrough)
        draw_text_decoration(rs, last, last_pen_x, local_pen_x, y / surface_scale, color);
      last = font;
      last_pen_x = pen_x;
    }
                        ...
                        size_t len, float x, int y, RenColor color)
{
  ...
  double pen_x = x * surface_scale;
  double pen_x_start = pen_x;         /* remember start‑of‑run for underline */
  ...
  double pen_x_end = pen_x;

  /* Draw underline / strikethrough for the whole shaped run */
  draw_text_decoration(rs, fonts[0], pen_x_start, pen_x_end, y, color);

  return pen_x_end / surface_scale;
}
