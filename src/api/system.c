...
top:
  ...
  // As per the SDL3 wiki, each event type is handled directly in the switch.
  ...
  switch (e.type) {
    ...
    case SDL_EVENT_WINDOW_RESIZED:
      ...
      // CORRECTED: In SDL3, for a resize event, the new size is in data1 and data2.
      lua_pushinteger(L, e.window.data1);
      lua_pushinteger(L, e.window.data2);
      return 3;
    
    case SDL_EVENT_WINDOW_PIXEL_SIZE_CHANGED:
      // This event also uses data1 and data2 for the new pixel dimensions.
      ren_resize_window(&window_renderer);
      lua_pushstring(L, "resized");
      // This will push the new size in *pixels*, which may be different from points on high-DPI displays.
      // Your application logic should decide if it wants points (from RESIZED) or pixels (from this event).
      // For now, we treat it the same as RESIZED.
      SDL_GetWindowSize(window_renderer.window, &w, &h);
      lua_pushinteger(L, w);
      lua_pushinteger(L, h);
      return 3;
