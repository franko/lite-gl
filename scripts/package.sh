...
main() {
  ...
  if [[ $source == true ]]; then source_package "lite-gl$version-src"; fi
  ...
  local exe_file="$(pwd)/${dest_dir}/lite-gl"

  local package_name=lite-gl$version-$platform-$arch
  local bundle=false
  ...
  if [[ -d "${data_dir}" ]]; then
    ...
    if [[ -d "${data_dir}" ]]; then
      ...
      rm -rf "Lite GL.app"; mv "${dest_dir}" "Lite GL.app"
      dest_dir="Lite GL.app"
      exe_file="$(pwd)/${dest_dir}/Contents/MacOS/lite-gl"
      data_dir="$(pwd)/${dest_dir}/Contents/Resources"
  ...
  if [[ $bundle == false && $portable == false ]]; then
    data_dir="$(pwd)/${dest_dir}/$prefix/share/lite-gl"
    exe_file="$(pwd)/${dest_dir}/$prefix/bin/lite-gl"
  fi
